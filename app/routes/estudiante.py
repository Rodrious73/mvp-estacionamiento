...
@estudiante_bp.route('/solicitar-pase', methods=['GET', 'POST'])
@login_required
@role_required('estudiante')
def solicitar_pase():
    user = get_current_user()
    ...
    return render_template('estudiante/solicitar_pase.html', 
                         user=user,
                         user_type='estudiante',
                         vehiculos=vehiculos,
                         ciclo_actual=ciclo_actual)

@estudiante_bp.route('/vehiculos', methods=['GET', 'POST'])
@login_required
@role_required('estudiante')
def vehiculos():
    user = get_current_user()
    ...
    if request.method == 'POST':
        try:
            ...
            if not all([placa, marca, modelo, tipo, color]):
                flash('Todos los campos son obligatorios', 'error')
                return redirect(url_for('estudiante.vehiculos'))
    ...
    # GET request
    vehiculos = Vehiculo.query.filter_by(usuario_id=user['id']).all()
    return render_template('estudiante/vehiculos.html', 
                         user=user, 
                         vehiculos=vehiculos)

@estudiante_bp.route('/eliminar-vehiculo', methods=['POST'])
@login_required
@role_required('estudiante')
...
@estudiante_bp.route('/solicitud/<int:id>')
@login_required
@role_required('estudiante')
def ver_solicitud(id):
    user = get_current_user()
    solicitud = SolicitudPase.query.filter_by(id=id, usuario_id=user['id']).first_or_404()
    return render_template('estudiante/ver_solicitud.html', 
                         user=user, 
                         solicitud=solicitud)

@estudiante_bp.route('/pase/<int:id>')
@login_required
@role_required('estudiante')
def ver_pase(id):
    user = get_current_user()
    pase = PaseVehicular.query.filter_by(id=id, usuario_id=user['id']).first_or_404()
    return render_template('estudiante/ver_pase.html', 
                         user=user, 
                         pase=pase)

@estudiante_bp.route('/cancelar-solicitud', methods=['POST'])
@login_required
@role_required('estudiante')
